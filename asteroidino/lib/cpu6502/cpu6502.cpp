...
void mos6502::NMI(bool line)
{
   ...
   if (nmi_line == true && line == false) {
      if (!nmi_inhibit) {
         ...
         static int nmi_debug = 0;
         if (nmi_debug++ < 5) {
            // Note: Can't use Serial here - wrong context
            // Will add flag to check in main loop
         }
      }
   }
   nmi_line = line;
...
{
   // Debug: Track NMI service calls
   static int svc_nmi_count = 0;
   svc_nmi_count++;
